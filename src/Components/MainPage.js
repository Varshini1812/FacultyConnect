...
function Main_page(props) {
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      alert('Error during sign-in with Google. Please check the console for more details.');
    }
  };

  return (
    <div id='main'>
      <h1 id='mhead'>FACULTY FEEDBACK SYSTEM</h1>
      <Link id='k' to="/adlogin">
        <button id="mbut" value="ADMIN" onClick={() => setUser('ADMIN')}>
          <img id='adimg' src={ac} alt="noimage"></img>
          <h3>ADMIN</h3>
        </button>
      </Link>
